...
struct _blink_plugin_data
{
uint16_t delay_ms;
uint8_t total_blinks;
};
typedef struct _blink_plugin_data blink_plugin_data;
static inline blink_plugin_data blink_plugin_parser_read(ZEPTO_PARSER* sa_po)
{
...
static inline void blink_plugin_reply_write(REPLY_HANDLE sa_rh, uint8_t made_blinks)
{
papi_reply_write_byte(sa_rh, made_blinks);
}
struct _blink_plugin_config
{
uint8_t pin_led;
};
typedef struct _blink_plugin_config blink_plugin_config;
